...
class TreeStore extends CascadeStore_1.CascadeStore {
    ...
    async doOperation(entity, operation, context, params) {
        ...
        if (action === 'select') {
            const rows = await this.cascadeSelect(entity, operation, context, params);
            const result = await this.formResult(entity, rows, operation, context, params);
            const ids = result.map((ele) => ele.id);
            return { ids };
        }
        else {
            await this.cascadeUpdate(entity, operation, context, params);
            return {};
        }
    }
    ...
    async formResult(entity, rows, selection, context, params, nodeDict) {
        ...
        const copyExprNode = (entity2, proj, sort) => {
