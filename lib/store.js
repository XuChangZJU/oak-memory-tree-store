...
var TreeStore = /** @class */ (function (_super) {
    ...
    TreeStore.prototype.translateLogicFilter = function (entity, projection, filter, attr, context, option) {
        var _this = this;
        var self = [];
        var otm = [];
        var mto = [];
        switch (attr) {
            case '$and': {
                ...
                var fns = filters.map(function (ele) { return _this.translateFilterInner(entity, projection, ele, context, option); });
            ...
            case '$or': {
                var filters = filter[attr];
                var fns_1 = filters.map(function (ele) { return _this.translateFilterInner(entity, projection, ele, context, option); });
                /**
            ...
            case '$not': {
                ...
                var filterFn_1 = this.translateFilterInner(entity, projection, filter2, context, option);
                var fn = function (node, nodeDict, exprResolveFns) {
    ...
    TreeStore.prototype.translateFilterInner = function (entity, projection, filter, context, option) {
        var _this = this;
        var self = [];
        var otm = [];
        var mto = [];
        ...
        var _loop_1 = function (attr) {
            ...
            if (attr === '#id') {
                ...
                var filterFns = this_1.translateLogicFilter(entity, projection, filter, attr, context, option);
                ...
                if (relation_2 === 1) {
                    ...
                    var filterFn_2 = this_1.translateFilter(attr, projection[attr] || {}, filter[attr], context, option);
                    mto.push(function (node, nodeDict, exprResolveFns) {
                        ...
                        if (row.entityId === undefined || row.entity === undefined) {
                            ...
                            if (option === null || option === void 0 ? void 0 : option.ignoreAttrMiss) {
                                if (process.env.NODE_ENV === 'development') {
                                    console.warn("\u5BF9\u8C61".concat(entity, "\u4E0A\u7684entity/entityId\u4E0D\u80FD\u786E\u5B9A\u503C\uFF0C\u53EF\u80FD\u4F1A\u5F71\u54CD\u5224\u5B9A\u7ED3\u679C"));
                                }
                    ...
                    // 只能是基于普通属性的外键
                    var filterFn_3 = this_1.translateFilter(relation_2, projection[attr] || {}, filter[attr], context, option);
                    mto.push(function (node, nodeDict, exprResolveFns) {
                        var row = _this.constructRow(node, context, option);
                        if (!row) {
                            return false;
                        ...
                        if (row["".concat(attr, "Id")] === undefined) {
                            // 说明一对多的外键没有取出来，需要抛出RowUnexists异常
                            (0, assert_1.assert)(typeof projection[attr] === 'object');
                            if (option === null || option === void 0 ? void 0 : option.ignoreAttrMiss) {
                                if (process.env.NODE_ENV === 'development') {
                                    console.warn("\u5BF9\u8C61".concat(entity, "\u4E0A\u7684").concat(attr, "Id\u4E0D\u80FD\u786E\u5B9A\u503C\uFF0C\u53EF\u80FD\u4F1A\u5F71\u54CD\u5224\u5B9A\u7ED3\u679C"));
                                }
                                return false; // 若不能确定，认定为条件不满足                                    
                            }
                            throw new Exception_1.OakRowUnexistedException([{
                        ...
                        (0, assert_1.assert)(row["".concat(attr, "Id")] === null);
                        return false;
                    });
                }
                    ...
                    else if (!(option === null || option === void 0 ? void 0 : option.disableSubQueryHashjoin)) {
                        /**
                         * 尝试用hashjoin将内表数组取出，因为memory中的表都不会太大，且用不了索引（未来优化了可能可以用id直接取值），因而用hash应当会更快
                         */
                        var option2 = Object.assign({}, option, { dontCollect: true });
                        try {
                            var subQueryRows = this_1.selectAbjointRow(otmEntity_1, {
                                data: otmProjection_1,
                                filter: filter[attr],
                            }, context, option2);
                            var buckets_1 = (0, lodash_1.groupBy)(subQueryRows, fk_1);
                            otm.push(function (node, nodeDict) {
                                var _a, _b;
                                var row = _this.constructRow(node, context, option);
                                if (!row) {
                                ...
                                switch (predicate_1) {
                                    case 'in': {
                                        return (((_a = buckets_1[row.id]) === null || _a === void 0 ? void 0 : _a.length) > 0);
                                    }
                                    case 'not in': {
                                        return (!buckets_1[row.id] || buckets_1[row.id].length === 0);
                                    }
                                    case 'all': {
                                        return (((_b = buckets_1[row.id]) === null || _b === void 0 ? void 0 : _b.length) > 0 && Object.keys(buckets_1).length === 1);
                                    }
                                    case 'not all': {
                                        return Object.keys(buckets_1).length > 1 || !buckets_1.hasOwnProperty(row.id);
                                    }
                                    default: {
                                        (0, assert_1.assert)(false, "unrecoganized sqp operator: ".concat(predicate_1));
                                    }
                                }
                            });
                        }
    ...
    TreeStore.prototype.translateFilter = function (entity, projection, filter, context, option) {
        var _this = this;
        var filterFns = this.translateFilterInner(entity, projection, filter, context, option);
     ...
     * @returns
     */
    TreeStore.prototype.getEntityNodes = function (entity, selection, context) {
        var filter = selection.filter;
        var ids = (0, filter_1.getRelevantIds)(filter);
        if (this.store[entity]) {
            if (ids.length > 0) {
                var entityNodes = (0, lodash_1.pick)(this.store[entity], ids);
    ...
    TreeStore.prototype.selectAbjointRow = function (entity, selection, context, option) {
        var e_9, _a, e_10, _b;
        var _this = this;
        var data = selection.data, filter = selection.filter;
        var nodeDict = option === null || option === void 0 ? void 0 : option.nodeDict;
        var filterFn = filter && this.translateFilter(entity, data, filter, context, option);
        var entityNodes = this.getEntityNodes(entity, selection, context);
        var nodes = [];
        try {
    ...
    TreeStore.prototype.formResult = function (entity, rows, selection, context, option) {
        ...
        if (incompletedRowIds.length > 0) {
            ...
            if (option === null || option === void 0 ? void 0 : option.ignoreAttrMiss) {
                if (process.env.NODE_ENV === 'development') {
                    console.warn("\u5BF9\u8C61".concat(entity, "\u4E0A\u6709\u5C5E\u6027\u7F3A\u5931\uFF0C\u53EF\u80FD\u4F1A\u5F71\u54CD\u4E0A\u5C42\u4F7F\u7528\u7ED3\u679C\uFF0C\u8BF7\u786E\u5B9A"));
                }
            }
            else {
                throw new Exception_1.OakRowUnexistedException([{
